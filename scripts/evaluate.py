...
def evaluate(args):
    ...
    with open(args.output_csv, 'w', newline='') as f:
        ...
        writer.writerow([
            'cif_id', 'atom_index',
            'true_dx', 'true_dy', 'true_dz',
            'pred_dx', 'pred_dy', 'pred_dz',
            'error_mag'
        ])

        with torch.no_grad():
            for ((atom_fea, nbr_fea, nbr_idx, crystal_atom_idx),
                 dr_true, struct_relaxed) in loader:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', required=True,
                        help='Path to model_best.pth.tar checkpoint')
    parser.add_argument('--output-csv', default='test_atom_errors.csv',
