...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
    ...
    if train_ratio is None:
        ...
        warnings.warn(f"train_ratio is None, using 1 - val_ratio - test_ratio = {train_ratio} as training data.")
    else:
    ...
    if return_test:
        test_sampler = SubsetRandomSampler(indices[train_size+valid_size:train_size+valid_size+test_size])
...
def collate_pool(dataset_list):
    ...
    for (atom_fea, nbr_fea, nbr_idx), dr_true, struct_relaxed in dataset_list:
        ...
        crystal_atom_idx.append(torch.arange(base_idx, base_idx + n_i, dtype=torch.long))
...
class CIFData(Dataset):
    ...
    """
    def __init__(self, root_dir, max_num_nbr=12, radius=8.0,
    ...
    def _featurize(self, crystal):
        ...
        for nbr in all_nbrs:
            if len(nbr) < self.max_num_nbr:
                ...
                dists = [x[1] for x in nbr] + [self.radius+1]*(self.max_num_nbr - len(nbr))
            else:
